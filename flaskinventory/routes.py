...
def overview():
    ...
    if exists == False:
        flash(f'Add books,participating students and make transfers between students', 'info')
...
def product():
    ...
    if exists == False and request.method == 'GET':
        ...
        try:
            db.session.commit()
            flash(f'Your book has been updated!', 'success')
        ...
        try:
            db.session.commit()
            flash(f'Your book {form.prodname.data} has been added!', 'success')
...
def loc():
    ...
    if lform.validate_on_submit() and request.method == 'POST':
        ...
        try:
            db.session.commit()
            flash(f'This Student has been updated!', 'success')
...
def move():
    ...
    if exists == False and request.method == 'GET':
        flash(f'Transfer books  to view', 'info')
    # ----------------------------------------------------------
    ...
    if form.validate_on_submit() and request.method == 'POST':
        ...
        if boolbeans == False:
            ...
            flash(f'Not enough book from this student .Please add more books', 'danger')
        else:
...
def delete():
    ...
    if type == 'product':
        ...
        db.session.commit()
        flash(f'This book has been deleted!', 'success')
        return redirect(url_for('product'))
        ...
        db.session.commit()
        flash(f'This student has been deleted!', 'success')
        return redirect(url_for('loc'))
